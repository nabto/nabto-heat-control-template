...
/*
  <query name="getPublicDeviceInfo"  id="1000">
    <request>
    </request>
    <response format="json">
      <parameter name="deviceName" type="raw"/>
      <parameter name="deviceType" type="raw"/>
      <parameter name="deviceIcon" type="raw"/>
    </response>
  </query>
 */
    case 1000: {
        const char* deviceName = "Apollovej Stuen 3";
        const char* deviceType = "ACME 9001 Heatpump";
        const char* deviceIcon = "img/chip-small.png";
        if (!write_string(write_buffer, deviceName)) return AER_REQ_RSP_TOO_LARGE;
        if (!write_string(write_buffer, deviceType)) return AER_REQ_RSP_TOO_LARGE;
        if (!write_string(write_buffer, deviceIcon)) return AER_REQ_RSP_TOO_LARGE;
        return AER_REQ_RESPONSE_READY;
    }
